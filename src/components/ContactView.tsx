...
import { 
  ...
  CheckCircle, ShieldCheck, Building2, ChevronRight, Compass, Landmark 
} from "lucide-react";
import { InquiryFormState } from "../types";

interface ContactViewProps {
  initialBrandInterest?: string;
}

export default function ContactView({ initialBrandInterest = "" }: ContactViewProps) {
  // Office location visual coordinates simulation
  ...
  const handleBrandSelect = (brandName: string) => {
    setFormData((prev) => {
      ...
      if (alreadySelected) {
        return {
          ...prev,
          brandsOfInterest: prev.brandsOfInterest.filter((b) => b !== brandName)
        };
      } else {
        return {
          ...prev,
          brandsOfInterest: [...prev.brandsOfInterest, brandName]
        };
      }
    });
  };
  ...
  return (
    <div className="bg-slate-50 min-h-screen">
      ...
      {/* 2. Form & Contacts Split Section */}
      <section className="py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          
          {/* Left panel: Contact cards & directions simulation */}
          <div className="lg:col-span-5 space-y-8">
            
            {/* Engineered for Security Badge */}
            <div className="bg-slate-900 text-white rounded-2xl p-6 sm:p-8 space-y-4 border border-slate-800 shadow-lg">
              <div className="flex gap-3.5 items-center">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-mono text-slate-550 uppercase">AUTHORIZED ACGR SEAL</p>
                </div>
              </div>
            ...
            {/* Interactive branch office address card */}
            <div className="bg-white rounded-3xl border border-slate-205 p-6 sm:p-8 shadow-sm space-y-6">
              ...
              <div className="grid grid-cols-3 gap-1.5 p-1 bg-slate-100 rounded-xl">
                {[
                  ...
                  <button
                    key={off.id}
                    onClick={() => setSelectedOffice(off.id as any)}
              ...
              <div className="space-y-4 pt-2">
                ...
                <div className="p-3 bg-brand-50/50 rounded-xl border border-brand-100 flex gap-2 text-[11px] text-slate-600 leading-relaxed items-start">
                  <Compass className="w-4 h-4 text-brand-605 mt-0.5 shrink-0" />
                  <div>
          ...
          {/* Right panel: Inquiry Intake form */}
            ...
            {!formSubmitted ? (
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className={`w-full p-3 bg-slate-50 border rounded-xl outline-none text-sm transition-all text-slate-800 ${
                        formErrors.name ? "border-red-500 bg-red-50/20" : "border-slate-250 focus:border-brand-500 focus:bg-white"
                      }`}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      type="tel"
                      className={`w-full p-3 bg-slate-50 border rounded-xl outline-none text-sm transition-all text-slate-800 ${
                        formErrors.contactNo ? "border-red-500 bg-red-50/20" : "border-slate-250 focus:border-brand-500 focus:bg-white"
                      }`}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      type="email"
                      className={`w-full p-3 bg-slate-50 border rounded-xl outline-none text-sm transition-all text-slate-800 ${
                        formErrors.email ? "border-red-500 bg-red-50/20" : "border-slate-250 focus:border-brand-500 focus:bg-white"
                      }`}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      type="text"
                      className={`w-full p-3 bg-slate-50 border rounded-xl outline-none text-sm transition-all text-slate-800 ${
                        formErrors.city ? "border-red-500 bg-red-50/20" : "border-slate-250 focus:border-brand-500 focus:bg-white"
                      }`}
                  ...
                  <div className="space-y-1.5 sm:col-span-2">
                    ...
                    <select
                      className={`w-full p-3 bg-slate-50 border rounded-xl outline-none text-sm transition-all text-slate-800 ${
                        formErrors.userType ? "border-red-500 bg-red-50/20" : "border-slate-250 focus:border-brand-500 focus:bg-white"
                      }`}
                      ...
                      return (
                        <button
                          ...
                          className={`p-2.5 rounded-xl border text-xs font-sans text-left font-semibold transition-all flex items-center justify-between ${
                            isChecked
                              ? "bg-brand-550 border-brand-500 text-white"
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    rows={4}
                    className="w-full p-3 bg-slate-50 border border-slate-250 focus:border-brand-500 focus:bg-white rounded-xl outline-none text-sm transition-all text-slate-800 placeholder:text-slate-400"
                ...
                <button
                  type="submit"
                  className="w-full py-4 bg-brand-600 hover:bg-brand-550 text-white font-display font-semibold rounded-xl text-sm transition-colors duration-200 flex items-center justify-center gap-2 shadow"
                >
              ...
              // IF FORM IS SUBMITTED (THANK YOU STATE)
              <motion.div 
              ...
              >
                ...
                <div className="p-5 bg-slate-50 border rounded-2xl max-w-md mx-auto space-y-3">
                  <div>
                    <span className="text-[10px] font-mono text-slate-400 block font-bold uppercase uppercase">REFERENCE TICKET:</span>
                  ...
                  <div className="pt-2.5 border-t border-slate-200">
                    <span className="text-[10px] font-mono text-slate-400 block font-bold uppercase uppercase">ASSIGNED REPRESENTATIVE desk:</span>
                ...
                <div className="pt-4 flex flex-col sm:flex-row gap-3 justify-center">
                  <button
                    ...
                    className="px-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-705 border font-display font-semibold rounded-xl text-xs transition-colors"
                  >
